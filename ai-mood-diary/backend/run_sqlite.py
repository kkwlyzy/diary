import sys
sys.path.insert(0, '.')

print("Starting with SQLite for testing...")

import os
os.environ['ARK_API_KEY'] = 'ark-008c7019-c676-43cd-98c3-967476ca543b-56875'

from sqlalchemy import create_engine, text
from sqlalchemy.orm import sessionmaker, declarative_base

SQLITE_URL = "sqlite:///./test.db"
engine = create_engine(SQLITE_URL, connect_args={"check_same_thread": False})
SessionLocal = sessionmaker(autocommit=False, autoflush=False, bind=engine)
Base = declarative_base()

from app.models.user import User
from app.models.diary import Diary
from app.models.emotion import EmotionAnalysis

Base.metadata.create_all(bind=engine)
print("Tables created")

from fastapi import FastAPI
from fastapi.middleware.cors import CORSMiddleware
from app.routers import auth, diary, statistics, ai, captcha

app = FastAPI(title="AI心情日记", description="AI Mood Diary API", version="1.0.0")

app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
    allow_credentials=True,
    allow_methods=["*"],
    allow_headers=["*"],
    expose_headers=["X-Captcha-Session"],
)

app.include_router(auth.router)
app.include_router(diary.router)
app.include_router(statistics.router)
app.include_router(ai.router)
app.include_router(captcha.router)

@app.get("/")
def root():
    return {"message": "AI心情日记 API 服务运行中 (SQLite)"}

print("App ready, starting uvicorn...")
import uvicorn
uvicorn.run(app, host='0.0.0.0', port=8001, log_level='info')
